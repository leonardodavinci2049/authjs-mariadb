...
class DatabaseMigrator {
   ...
   */
  private async executeMigration(migrationFile: MigrationFile): Promise<boolean> {
  ...
  private isTableExistsError(error: unknown): boolean {
    ...
    if (error && typeof error === "object" && "erroOriginal" in error) {
      ...
      return customError.erroOriginal?.message?.includes("already exists") || false;
    }
  ...
  async runMigrations(): Promise<void> {
    ...
    try {
      ...
      for (const migrationFile of migrationFiles) {
        const alreadyExecuted = await this.isMigrationExecuted(migrationFile.filename);
        
        if (alreadyExecuted) {
          console.log(`⏭️  Migration ${migrationFile.filename} já foi executada`);
          skippedCount++;
          continue;
        }
...
async function main() {
  ...
  switch (command) {
    ...
    default:
      ...
      console.log("  npx tsx database/setup.ts migrate  # Executar migrations");
      console.log("  npx tsx database/setup.ts status   # Listar status");
      break;
  }
}
